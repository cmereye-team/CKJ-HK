...
const onSlideChange = (swiper: any) => {
  latestNewsCurrent.value =
    (swiper.realIndex ? Number(swiper.realIndex) : 0) + 1
}
let newsLists = ref([
  {
    title: '',
  ...
  if (res) {
    if (res.data && res.data.length) {
      newsLists.value = res.data.map((item) => {
        let _mbUrl =
          item.ext_mbImg_hk && `https://admin.ckjhk.com${item.ext_mbImg_hk}`
        let _imgUrl =
          item.ext_pcImg_hk && `https://admin.ckjhk.com${item.ext_pcImg_hk}`
...
<template>
  <div class="index-latestNews pageCon">
    ...
    >
      <swiper-slide
        v-for="(latestNewsItem, latestNewsIndex) in filterBanner()"
        :key="latestNewsIndex"
      >
        <div class="index-latestNews-in">
          <nuxt-link :to="latestNewsItem.link">
            <img
              class="newsImg"
              :src="
                windowWidth > 768 ? latestNewsItem.imgUrl : latestNewsItem.mbUrl
              "
    ...
    <div class="index-latestNews-line">
      <YahooPageSwiperPointLine
        :latestNewsNum="filterBanner().length"
        :latestNewsCurrent="latestNewsCurrent"
