...
import service from '~/assets/yahoo/js/service'
import type { FormInstance, FormRules } from 'element-plus'
import { useAppState } from '~~/stores/yahoo/appState'
import { smallPhoneNum } from '~/assets/yahoo/js/common'
...
const onSubmit = async () => {
  ...
  if (res) {
    if (res.code) {
      ...
      reForm()
      window.location.href = `/yahoo/messagePage?c=${res.code}`
    } else {
      ElMessage({
        showClose: true,
        message: res.data,
...
备注信息：服务器离线由备用服务推送`,
  ...
  if (data) {
    localStorage.setItem('contactForm', JSON.stringify(_form))
    reForm()
    window.location.href = `/yahoo/messagePage`
  } else {
    ElMessage({
      showClose: true,
      message: '服务异常，请稍后重试',
              ...
              <div class="privacyPolicy">
                ...
                >
                  ...
                  <span class="radio">
                    ...
                    <div>
                      <i>*</i>本人已閱讀並同意有關
                      <nuxt-link to="/yahoo/privacyPolicy?from=2" target="_blank"
                        >私隱政策</nuxt-link
                      >
                      聲明。
                    </div>
                ...
                >
                  ...
                  >
                    <span class="contactUsForm">
                      <img
                        class="contactUsForm"
                        src="@/assets/yahoo/images/icon_36.svg"
