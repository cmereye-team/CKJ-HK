...
import service from '~/assets/yahool/js/service'
import type { FormInstance, FormRules } from 'element-plus'
import { useAppState } from '~/stores/yahool/appState'
import { smallPhoneNum } from '~/assets/yahool/js/common'
...
const onSubmit = async () => {
  ...
  const { data }: any = await useFetch(
    'https://admin.ckjhk.com/api.php/cms//fcode/3',
    {
      method: 'post',
      body: _formData,
    }
  ...
  if (res) {
    if (res.code) {
      ...
      reForm()
      window.location.href = `/yahool/messagePage?c=${res.code}`
    } else {
      ElMessage({
        showClose: true,
        message: res.data,
...
备注信息：服务器离线由备用服务推送`,
  ...
  if (data) {
    localStorage.setItem('contactForm', JSON.stringify(_form))
    reForm()
    window.location.href = `/yahool/messagePage`
  } else {
    ElMessage({
      showClose: true,
      message: '服务异常，请稍后重试',
          ...
          >
              ...
              <div class="privacyPolicy">
                ...
                >
                  ...
                  <span class="radio">
                    ...
                    <div>
                      <i>*</i>本人已閱讀並同意有關
                      <nuxt-link to="/yahool/privacyPolicy?from=2" target="_blank"
                        >私隱政策</nuxt-link
                      >
                      聲明。
                    </div>
            ...
            <el-form-item>
              <div class="formBtn contactUsForm">
                <div
                  class="animbtntypetwo contactUsForm"
                  v-loading="formLoading"
                >
                  <div
                    class="animbtntypetwo-in contactUsForm"
                    :id="
                      windowWidth > 768 ? 'contactUsForm' : 'navMbContactForm'
                    "
                  ...
                  >
                    <span class="contactUsForm">
                      <img
                        class="contactUsForm"
                        src="@/assets/yahool/images/icon_36.svg"
