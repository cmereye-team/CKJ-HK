...
<template>
  <div class="process">
    ...
    <div class="process-context pageCon" :class="processData.pageName">
      <span
        v-for="(processContextItem, processContextIndex) in processData.context"
        :key="processContextIndex"
        >{{ processContextItem }}</span
      >
    </div>
    <div class="process-tabs pageCon" :class="processData.pageName">
      <div
        :class="{ active: processTabsActive === processTabIndex }"
        @click="handleProcessTabs(processTabIndex)"
        v-for="(processTabItem, processTabIndex) in processData.tabs"
        :key="processTabIndex"
      >
        {{ processTabItem }}
      </div>
    ...
    <div class="process-step pageCon">
      ...
      >
        <SwiperSlide
          v-for="(processItem, index) in processData.lists"
          :key="index"
        >
          <div class="step-in">
            <div
              class="step-in-box"
              v-for="(step, stepIndex) in processItem.listItem"
              :key="stepIndex"
            >
              <div class="step-itemLists">
                <div
                  class="step-itemLists-in"
                  v-for="(stepChild, stepChildIndex) in step"
                  :key="stepChildIndex"
                >
                  <div
                    :class="[
                      'line',
                      { lineNone: step.length === 2 && stepChildIndex === 1 },
                    ]"
                  ></div>
                  ...
                  <span class="text">
                    <div>
                      {{ stepChild.textMb ? stepChild.textMb : stepChild.text }}
                    </div>
                    <div class="text-in" v-if="stepChild.textIn">
                      {{ stepChild.textIn }}
                    </div>
                  </span>
                </div>
