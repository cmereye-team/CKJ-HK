  ...
  <div class="differ">
    <div class="dentistryServices-title">
      <div class="dentistryServices-title-in bb">
        <div v-if="windowWidth > 768 || differData.title.length <= 10">
          {{ differData.title }}
        </div>
        <div v-if="windowWidth <= 768 && differData.title.length > 10">
          {{ differData.title.slice(0, 6) }}
        </div>
        <div v-if="windowWidth <= 768 && differData.title.length > 10">
          {{ differData.title.slice(7, differData.title.length) }}
        </div>
      </div>
    </div>
    ...
    <div class="differ-in">
      <swiper
        :slidesPerView="windowWidth > 768 ? 2 : 1.1"
        class="swiper-wrapper"
        @slideChange="onSlideChange"
        @swiper="setDifferSwiperRef"
      >
        <swiper-slide :class="['swiper-slide', differData.pageName]">
          <div class="box">
            ...
            <div
              class="box-in"
              v-for="(leftItem, leftItemIndex) in differData.listDatas"
              :key="leftItemIndex"
            >
              <div :class="['box-in-l', differData.pageName]">
                {{ leftItem.name }}
              </div>
              <div :class="['box-in-r', differData.pageName]">
                ...
                <div
                  v-else
                  v-for="(firstTextItem, firstTextIndex) in leftItem.firstText"
                  :key="firstTextIndex"
                >
                  {{ firstTextItem }}
                </div>
        ...
        <swiper-slide :class="['swiper-slide', differData.pageName]">
          <div class="box">
            ...
            <div
              :class="['box-in', 'boxRight', differData.pageName]"
              v-for="(rightItem, rightItemIndex) in differData.listDatas"
              :key="rightItemIndex"
            >
    ...
    <div class="differ-line mbBox">
      <YahooPageSwiperPointLine
        :latestNewsNum="2"
        :latestNewsCurrent="differCurrent"
