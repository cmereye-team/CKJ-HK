...
const handleAreaTab = (_idx: number) => {
  ...
  if (props.isAdPage) {
    history.pushState({}, '', `/yahool/medical-team/${appState.areaTabs_url[_idx]}`)
  }
...
<template>
  <div class="areaTab">
    <div
      :class="appState.areaTabCurNum === areaTabIndex ? 'isCur' : ''"
      v-for="(item, areaTabIndex) in appState.areaTabs"
      :key="areaTabIndex"
      @click="handleAreaTab(areaTabIndex)"
    >
      {{ $t(item) }}
    </div>
  </div>
