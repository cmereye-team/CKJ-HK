...
import { zh_tran, zh_getLang, getCookie } from '~/assets/yahool/js/uselang'
import { useAppState } from '~/stores/yahool/appState'
import { toWhatsApp, smallPhoneNum } from '~/assets/yahool/js/common'
import serviceLists from '~/assets/yahool/js/service'
...
const classNamefilter = (_menu: any, _idx: number) => {
  let className = ''
  if (route.path === '/yahool/medical-team') {
    if (_menu.link.includes('medical-team')) {
      if (appState.areaTabCurNum === _idx) {
        className = 'menuChildCurrent'
      }
    }
  } else if (route.path.includes('/yahool/brand')) {
    if (_menu.link.includes(appState.brand)) {
      className = 'menuChildCurrent'
    }
  } else if (route.path.includes('/yahool/dental-service')) {
...
onMounted(() => {
  ...
  setTimeout(() => {
    if (route.path.includes('/cn/yahool')) {
      changlangsfun('s')
    }
  }, 500)
  isHealthCare()
...
const glangs = (_type) => {
  ...
  let _arr = [
    '/yahool/404',
    '/yahool/test',
    '/yahool/news-tooth-wiki',
    '/yahool/news-information',
    '/yahool/article',
  ]
  if (_arr.some((str) => route.path?.indexOf(str) !== -1)) {
    changlangsfun(_type)
  } else {
    if (_str === '/cn/yahool' || _str === '/hk/yahool') {
      let _url = route.path
      let _url_new = _url.replace(_url.slice(0, 3), _b ? '/yahool' + _b.lable : '')
      router.push(_url_new)
    } else {
      router.push(`${_b ? '/yahool' + _b.lable : ''}${route.path}`)
    }
  }
}
...
const isHealthCare = () => {
  if (route.path === '/yahool/dental-service/scaling-and-polishing') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/implant') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/fillings') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/veneers') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/toothtray') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/rootCanal') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/orthodontics') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/invisiblebraces') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/all-ceramic-crowns') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/children-dentistry') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/wisdom-teeth-extraction') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/teeth-whitening') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/general-oral-examination') {
    showExplain.value = true
  } else if (route.path == '/yahool/dental-service/periodontal') {
    showExplain.value = true
  } else if (route.path == '/yahool/health-care-voucher') {
    showExplain.value = true
  } else if (route.path == '/yahool/news/scaling-and-polishing-test') {
    showExplain.value = true
  } else if (route.path == '/yahool/news/news-tooth-wiki/263') {
    showExplain.value = true
  } else if (route.path == '/yahool/news/news-tooth-wiki/263') {
    showExplain.value = true
  } else {
...
const hideDiv = () => {
  console.log(route.path,'route.pathroute.pathroute.path');
  
  if (
    route.path == '/yahool/news/coverage' ||
    route.path == '/yahool/news/information' ||
    route.path == '/yahool/news/information/' ||
    route.path == '/yahool/news/tooth-wiki' ||
    route.path == '/yahool/messagePage' ||
    route.path == '/yahool/medical-team' ||
    route.path == '/yahool/contactUs' ||
    route.path == '/yahool/brand/course' ||
    route.path == '/yahool/federation-of-trade-unions-zone'
  ) {
    console.log('a');
...
<template>
  <header>
    <div class="header-content">
      ...
      >
        <YahooPageAnimBtnTypeTwo :str="btnText || '免費網上預約'" />
      </div>
      <div
        v-if="
        ...
        class="header-content-btn-implant"
      >
        <YahooPageAnimBtnTypeTwo :str="'獲取免費諮詢'" />
      </div>
      <div
        v-if="
          [
        ...
        >
          <div class="logo">
            <nuxt-link
              ...
              title="深圳愛康健口腔醫院"
              alt="深圳愛康健口腔醫院"
              ><img src="@/assets/yahool/images/logo_11.svg" alt=""
            /></nuxt-link>
          ...
          <div class="menu">
            ...
            >
              <nuxt-link
                :class="[
                  ...
                  {
                    'health-care-voucher':
                      menuItem.link === '/yahool/health-care-voucher',
                  },
                  {
                    'federation-of-trade-unions-zone':
                      menuItem.link === '/yahool/federation-of-trade-unions-zone',
                  },
                ]"
                :to="
                  ['/yahool/news', '/yahool/dental-service'].includes(menuItem.link)
                    ? 'javaScript:void(0)'
                    : menuItem.link
                "
              ...
              <div
                ...
                class="menuChild"
                :class="{
                  serviceCard: menuItem.link.includes('/yahool/dental-service'),
                }"
              >
                <div
                ...
                >
                  ...
                  >
                    {{
                      menuChildItem.link ===
                      '/yahool/dental-service/wisdom-teeth-extraction'
                        ? '拔牙\n'
                        : ''
                    }}
            ...
            <div class="menuItem langItem">
              <img src="@/assets/yahool/images/icon_26.svg" alt="" />
              <div class="menuChild">
                <div
                  :class="[
                    'menuChild-item',
          ...
          <div class="icon">
            <nuxt-link
              to="/yahool/health-care-voucher"
              class="health_care_voucher mb_gov_voucher"
            >
              <img src="@/assets/yahool/images/1321315134.svg" alt="" />
            </nuxt-link>
            <div class="icon-menuopen" @click="menuBoxBool = !menuBoxBool">
              <img v-if="!menuBoxBool" src="@/assets/yahool/images/icon_61.png" />
              <img v-else src="@/assets/yahool/images/icon_7.svg" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="menuBox" :style="{ top: menuBoxBool ? '0' : '-100vh' }">
        <div class="menuLists">
          <div
            ...
            :class="[
              ...
              { 'health-care-voucher': item.link === '/yahool/health-care-voucher' },
              {
                'federation-of-trade-unions-zone':
                  item.link === '/yahool/federation-of-trade-unions-zone',
              },
            ]"
          >
            <nuxt-link
        ...
        <div class="langItem">
          <img src="@/assets/yahool/images/icon_27.svg" alt="语言" />
          <span
            class="zh_click"
        ...
        <div class="menuBox-phone">
          <img src="@/assets/yahool/images/icon_11.svg" />（852）{{
        ...
        <div class="menuBox-icon">
          <!-- <div class="menuBox-icon-in">
            <nuxt-link
              ...
              target="_blank"
            >
              <img src="@/assets/yahool/images/icon_01.png" />
            </nuxt-link>
          </div> -->
          <div class="menuBox-icon-in">
            <nuxt-link to="https://www.instagram.com/ckj_hk/" target="_blank">
              <img src="@/assets/yahool/images/icon_02.svg" />
            </nuxt-link>
          </div>
          <div class="menuBox-icon-in">
            <nuxt-link
              ...
              target="_blank"
            >
              <img src="@/assets/yahool/images/icon_03.svg" />
            </nuxt-link>
          </div>
        </div>
      </div>
...
.waterBg::after {
  content: '';
  background-image: url(@/assets/yahool/images/back_wave01.png);
...
.waterBg::before {
  content: '';
  background-image: url(@/assets/yahool/images/back_wave03.png);
  background-repeat: repeat-x;
