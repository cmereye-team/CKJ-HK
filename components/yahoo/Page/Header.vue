...
import { zh_tran, zh_getLang, getCookie } from '~/assets/yahoo/js/uselang'
import { useAppState } from '~~/stores/yahoo/appState'
import { toWhatsApp, smallPhoneNum } from '~/assets/yahoo/js/common'
import serviceLists from '~/assets/yahoo/js/service'
...
const classNamefilter = (_menu: any, _idx: number) => {
  let className = ''
  if (route.path === '/yahoo/medical-team') {
    if (_menu.link.includes('medical-team')) {
      if (appState.areaTabCurNum === _idx) {
        className = 'menuChildCurrent'
      }
    }
  } else if (route.path.includes('/yahoo/brand')) {
    if (_menu.link.includes(appState.brand)) {
      className = 'menuChildCurrent'
    }
  } else if (route.path.includes('/yahoo/dental-service')) {
...
onMounted(() => {
  ...
  setTimeout(() => {
    if (route.path.includes('/cn/yahoo')) {
      changlangsfun('s')
    }
  }, 500)
  isHealthCare()
...
const glangs = (_type) => {
  ...
  if (_arr.some((str) => route.path?.indexOf(str) !== -1)) {
    ...
    if (_str === '/cn/yahoo' || _str === '/hk/yahoo') {
      let _url = route.path
      let _url_new = _url.replace(_url.slice(0, 3), _b ? '/yahoo' + _b.lable : '')
      router.push(_url_new)
    } else {
      router.push(`${_b ? '/yahoo' + _b.lable : ''}${route.path}`)
    }
  }
}
...
const isHealthCare = () => {
  if (route.path === '/yahoo/dental-service/scaling-and-polishing') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/implant') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/fillings') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/veneers') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/toothtray') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/rootCanal') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/orthodontics') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/invisiblebraces') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/all-ceramic-crowns') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/children-dentistry') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/wisdom-teeth-extraction') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/teeth-whitening') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/general-oral-examination') {
    showExplain.value = true
  } else if (route.path == '/yahoo/dental-service/periodontal') {
    showExplain.value = true
  } else if (route.path == '/yahoo/health-care-voucher') {
    showExplain.value = true
  } else if (route.path == '/yahoo/news/scaling-and-polishing-test') {
    showExplain.value = true
  } else if (route.path == '/yahoo/news/news-tooth-wiki/263') {
    showExplain.value = true
  } else if (route.path == '/yahoo/news/news-tooth-wiki/263') {
    showExplain.value = true
  } else {
...
const hideDiv = () => {
  if (
    route.path == '/yahoo/news/coverage' ||
    route.path == '/yahoo/news/information' ||
    route.path == '/yahoo/news/information/' ||
    route.path == '/yahoo/news/tooth-wiki' ||
    route.path == '/yahoo/messagePage' ||
    route.path == '/yahoo/medical-team' ||
    route.path == '/yahoo/contactUs' ||
    route.path == '/yahoo/brand/course' ||
    route.path == '/yahoo/federation-of-trade-unions-zone'
  ) {
    console.log('a');
  ...
  <header>
    <div class="header-content">
        ...
        >
          <div class="logo">
            <nuxt-link
              ...
              title="深圳愛康健口腔醫院"
              alt="深圳愛康健口腔醫院"
              ><img src="@/assets/yahoo/images/logo_11.svg" alt=""
            /></nuxt-link>
          ...
          <div class="menu">
            ...
            >
              <nuxt-link
                :class="[
                  ...
                  {
                    'health-care-voucher':
                      menuItem.link === '/yahoo/health-care-voucher',
                  },
                  {
                    'federation-of-trade-unions-zone':
                      menuItem.link === '/yahoo/federation-of-trade-unions-zone',
                  },
                ]"
                :to="
                  ['/yahoo/news', '/yahoo/dental-service'].includes(menuItem.link)
                    ? 'javaScript:void(0)'
                    : menuItem.link
                "
              ...
              <div
                ...
                class="menuChild"
                :class="{
                  serviceCard: menuItem.link.includes('/yahoo/dental-service'),
                }"
              >
                <div
                ...
                >
                  ...
                  >
                    {{
                      menuChildItem.link ===
                      '/yahoo/dental-service/wisdom-teeth-extraction'
                        ? '拔牙\n'
                        : ''
                    }}
            ...
            <div class="menuItem langItem">
              <img src="@/assets/yahoo/images/icon_26.svg" alt="" />
              <div class="menuChild">
                <div
                  :class="[
                    'menuChild-item',
          ...
          <div class="icon">
            <nuxt-link
              to="/yahoo/health-care-voucher"
              class="health_care_voucher mb_gov_voucher"
            >
              <img src="@/assets/yahoo/images/1321315134.svg" alt="" />
            </nuxt-link>
            <div class="icon-menuopen" @click="menuBoxBool = !menuBoxBool">
              <img v-if="!menuBoxBool" src="@/assets/yahoo/images/icon_61.png" />
              <img v-else src="@/assets/yahoo/images/icon_7.svg" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="menuBox" :style="{ top: menuBoxBool ? '0' : '-100vh' }">
        <div class="menuLists">
          <div
            ...
            :class="[
              ...
              { 'health-care-voucher': item.link === '/yahoo/health-care-voucher' },
              {
                'federation-of-trade-unions-zone':
                  item.link === '/yahoo/federation-of-trade-unions-zone',
              },
            ]"
          >
            <nuxt-link
        ...
        <div class="langItem">
          <img src="@/assets/yahoo/images/icon_27.svg" alt="语言" />
          <span
            class="zh_click"
        ...
        <div class="menuBox-phone">
          <img src="@/assets/yahoo/images/icon_11.svg" />（852）{{
        ...
        <div class="menuBox-icon">
          <!-- <div class="menuBox-icon-in">
            <nuxt-link
              ...
              target="_blank"
            >
              <img src="@/assets/yahoo/images/icon_01.png" />
            </nuxt-link>
          </div> -->
          <div class="menuBox-icon-in">
            <nuxt-link to="https://www.instagram.com/ckj_hk/" target="_blank">
              <img src="@/assets/yahoo/images/icon_02.svg" />
            </nuxt-link>
          </div>
          <div class="menuBox-icon-in">
            <nuxt-link
              ...
              target="_blank"
            >
              <img src="@/assets/yahoo/images/icon_03.svg" />
            </nuxt-link>
          </div>
        </div>
      </div>
...
.waterBg::after {
  content: '';
  background-image: url(@/assets/yahoo/images/back_wave01.png);
...
.waterBg::before {
  content: '';
  background-image: url(@/assets/yahoo/images/back_wave03.png);
  background-repeat: repeat-x;
