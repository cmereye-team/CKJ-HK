...
const getWindowResize = () => {
  if (window.innerWidth > 768) lineWidth.value = 80
  else lineWidth.value = 50
...
<template>
  <div
    ...
    :style="{
      width: isAutoWidth
        ? `calc(${lineWidth}px * ${latestNewsNum})`
        : 'calc(100% - 5px)',
    }"
  >
    <div class="boxLine" v-show="latestNewsNum !== 1"></div>
    <div
      class="boxLine-current"
      v-show="latestNewsNum !== 1"
      :style="{
        width: `calc(${
          ((latestNewsCurrent - 1) * 100) / (latestNewsNum - 1)
        }% - 5px)`,
      }"
    ></div>
    <div class="boxRound">
      <div
        class="boxRound-in"
        v-for="boxRoundIndex in latestNewsNum"
        :key="boxRoundIndex"
        @click="handleSwiperItem(boxRoundIndex)"
      >
